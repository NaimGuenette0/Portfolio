    ...
    public class PostsController : ControllerBase
    {
        ...
        public async Task<ActionResult<PostDisplayDTO>> PostPost(int hubId)
        {
            ...
            try
            {
                ...
                if (title == null) return BadRequest(new { Message = "Aucun titre trouvé" });
                if (text == null) return BadRequest(new { Message = "Aucun texte trouvé" });


                Comment? mainComment = await _commentService.CreateComment(user, text, null, null);
                if (mainComment == null) return StatusCode(StatusCodes.Status500InternalServerError);

                List<Picture>? pictures = await _pictureService.CreatePicture(formCollection, mainComment);
                if (pictures == null) return StatusCode(StatusCodes.Status500InternalServerError);

                mainComment.Pictures = pictures;
            ...
            {
                return RedirectToAction("PostPost");
            }
        ...
        public async Task<ActionResult<IEnumerable<PostDisplayDTO>>> GetHubPosts(int hubId, string sorting)
        {
            ...
            foreach (PostDisplayDTO p in posts)
            {
                Post? post = await _postService.GetPost(p.Id);
                foreach (Picture pic in post.MainComment.Pictures)
                {
                    p.MainComment.PictureIds.Add(pic.Id);
                }
            }
            return Ok(posts);
        }
        ...
        public async Task<IActionResult> SavePost(int id)
        {
            ...
            if (post == null) return NotFound();
            Post newPost = await _postService.SavePost(user, post);

            return Ok(new PostDisplayDTO(post, false, user));
        }

        [HttpGet]
        [Authorize]
        public async Task<ActionResult<IEnumerable<PostDisplayDTO>>> GetPostFavoris()
        {
            User? user = await _userManager.FindByIdAsync(User.FindFirstValue(ClaimTypes.NameIdentifier)!);
            if (user == null) return Unauthorized();

            IEnumerable<PostDisplayDTO>? posts = user.SavedPosts.Select(p => new PostDisplayDTO(p, false, user));
