...
namespace RaiditeServer.DTOs
{
    ...
    public class CommentDisplayDTO
    {
        ...
        public List<CommentDisplayDTO> SubComments { get; set; } = new List<CommentDisplayDTO>();
        ...
        public CommentDisplayDTO(Comment comment, bool withSubComments, User? user)
        {
            ...
            IsAuthor = user?.UserName == comment.User?.UserName && comment.User != null;
            PictureIds = comment.Pictures?.Select(p => p.Id).ToList() ?? new List<int>();
        }
    }
}
